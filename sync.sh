...
#    print_style "   install" "info"; printf "\t\t Installs docker-sync gem on the host machine.\n"
    ...
    print_style "   down" "success"; printf "\t\t\t remove compose services containers and network.\n"
    print_style "   bash" "success"; printf "\t\t\t Opens bash on the workspace with user laradock.\n"
    print_style "   reload" "success"; printf "\t\t\t Restart Docker Compose .\n"
...
#    docker-sync start;
    ...
    docker-compose up -d ${@} &&  docker-compose  exec php-fpm /bin/sh -c 'echo 172.19.0.1 tm.wx.com >> /etc/hosts'

elif [ "$1" == "down" ]; then
#    print_style "Stopping Docker Compose\n" "info"
    print_style "remove Docker Compose\n" "info"
    docker-compose down
...
#    docker-sync clean
   ...
   docker-compose restart nginx mysql php-fpm redis php-worker rabbitmq kibana && docker-compose  exec php-fpm /bin/sh -c 'echo 172.19.0.1 tm.wx.com >> /etc/hosts'
